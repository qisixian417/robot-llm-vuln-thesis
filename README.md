...
- **检测效果**：RAG+LLM F1=0.71（注：传统静态工具如Cppcheck/Semgrep需要完整编译单元，在函数级片段上无法正常运行，因此不适用于本任务场景）
...
- **关键发现**：CWE-476最高频(27.7%)，core_middleware组件中83%的样本为漏洞样本
...
- **数据独立性**：✅ 测试集和RAG完全独立（按仓库切分，可通过 `scripts/verify_data_independence.py` 验证）
...
- **静态工具**：在函数级片段上无法运行（需完整编译单元），不适用于本任务
- **提升**：RAG+LLM实现了函数级漏洞检测，填补了静态工具的空白
